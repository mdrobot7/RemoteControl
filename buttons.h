...
void handleButtons()
{
	...
	refresh(0);
	long long nowPlusDelay = now + repeatDelays(lastButton, 0);
	...
	{
		if (duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count() <= nowPlusDelay) return; //if the timer hasn't expired yet, return the function and try again
	...
	if (controller.Gamepad.wButtons != 0 || button(XINPUT_GAMEPAD_LTRIGGER) || button(XINPUT_GAMEPAD_RTRIGGER))
	{
		for (int i = 0; i < sizeof(_xinputButtonCodes); i++)
		{
			if (button(_xinputButtonCodes[i]))
			{
				...
				{
					/*SystemParametersInfoA(SPI_SETMOUSESONAR, 0, true, SPIF_UPDATEINIFILE);
					doInput(VK_LCONTROL, true);
					SystemParametersInfoA(SPI_SETMOUSESONAR, 0, false, SPIF_UPDATEINIFILE);*/
				}
